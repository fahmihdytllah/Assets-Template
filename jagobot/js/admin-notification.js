$(document).ready(function () {
  ...
  $(document).on('click', '.Notification-delete', function () {
    ...
    Swal.fire({ text: 'Apakah anda yakin ingin menghapus Notification ini?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes', customClass: { confirmButton: 'btn btn-primary waves-effect waves-light', cancelButton: 'btn btn-outline-danger ms-2 waves-effect waves-light' }, buttonsStyling: false }).then(function (result) {
      if (result.value) {
  ...
  formAddNotification.submit(function (e) {
    e.preventDefault();
    formAddNotification.block({ message: itemLoader, css: { backgroundColor: 'transparent', border: '0' }, overlayCSS: { backgroundColor: '#fff', opacity: 0.8 } });
    $.ajax({
  ...
  formEditNotification.submit(function (e) {
    e.preventDefault();
    formEditNotification.block({ message: itemLoader, css: { backgroundColor: 'transparent', border: '0' }, overlayCSS: { backgroundColor: '#fff', opacity: 0.8 } });
    $.ajax({
  ...
  formSendNotification.submit(function (e) {
    e.preventDefault();
    formSendNotification.block({ message: itemLoader, css: { backgroundColor: 'transparent', border: '0' }, overlayCSS: { backgroundColor: '#fff', opacity: 0.8 } });
    $.ajax({
      url: '/admin/notifications/',
      type: 'PATCH',
      data: $(this).serialize(),
    ...
    $.get('/admin/notifications?type=json', function (data) {
      ...
      data.forEach(notif => {
        ...
        $('#listNotifications').append(`<tr>
          <td><i class="ti ti-${icons[notif.icon]} ti-lg text-${notif.icon} me-3"></i> <span class="fw-medium">${notif.title}</span></td>
          <td>${notif.message}</td>
          ...
          <td>
            <div class="dropdown">
              ...
              <div class="dropdown-menu">
                <a class="dropdown-item Notification-edit" data-id="${notif._id}" ><i class="ti ti-pencil me-1"></i> Edit</a>
                <a class="dropdown-item Notification-delete" data-id="${notif._id}" ><i class="ti ti-trash me-1"></i> Delete</a>
              </div>
            </div>
          </td>
        </tr>`);
