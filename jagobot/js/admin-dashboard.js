  ...
  function loadDataDiagram() {
    $.get('/api/dashboard-admin', function (data) {
      if (data.status) {
        ...
        $('.totalBots').html(data.totalBots);
        $('.totalTransactions').html(data.totalTransactions);

        if (typeof radialBarChartEl !== undefined && radialBarChartEl !== null) {
          const radialChart = new ApexCharts(radialBarChartEl, radialBarChartConfig(data.keyStatistics.labels, data.keyStatistics.series));
          radialChart.render();
        }

        if (typeof donutChartEl !== undefined && donutChartEl !== null) {
          const donutChart = new ApexCharts(donutChartEl, donutChartConfig(data.keyRatio.labels, data.keyRatio.series));
          donutChart.render();
        }
      }
    });
