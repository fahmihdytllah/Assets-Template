$(function () {
  let dataPostsTable = $('.datatables-basic'),
    ...
    pageToken = $('#selectPage').val().split('[]')[1]

  $('#selectPage').change(function () {
    const val = $(this).val().split('[]')
    pageToken = val[1]
    dataPosts.ajax.url(`?type=json&pageId=${val[0]}&pageToken=${pageToken}`).load()
  })

  $('.btn-login').click(function () {
    window.location.href = '/oauth/facebook_grant_access'
  })

  $('.btn-logout').click(function () {
    Swal.fire({
      text: 'Are you sure you want to log out of your Facebook account?',
      icon: 'warning',
      showCancelButton: !0,
      ...
      if (e.value) {
        window.location.href = '/oauth/logout/facebook'
      }
    })
  })
