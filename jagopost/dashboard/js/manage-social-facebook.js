$(function () {
  ...
  let dataPosts,
    pageToken = elPage.val()?.split('[]')[1];

  $('#selectPage').change(function () {
    const [_pageId, _pageToken] = $(this).val().split('[]');
    pageToken = _pageToken;
    dataPosts.ajax.url('?type=json&pageId=' + _pageId + '&pageToken=' + _pageToken).load();
  });

  $('.btn-login').click(function () {
    Swal.fire({
      title: 'Are you sure?',
      html: 'Want to connect to your <strong>Facebook</strong> account',
      icon: 'question',
  ...
  $('.btn-logout').click(function () {
    Swal.fire({
      title: 'Are you sure?',
      html: 'Want to remove the connection to your <strong>Facebook</strong> account',
      icon: 'question',
      showCancelButton: true,
      ...
      if (result.value) {
        thirdPartyRemoveAccess('facebook', {
          onSuccess: (res) => {
            Swal.fire({
              title: 'Good News!',
              text: res.msg,
  ...
  if (dataPostsTable.length) {
    dataPosts = dataPostsTable.DataTable({
      ...
      columns: [{ data: '' }, { data: 'message' }, { data: 'created_time' }, { data: 'id' }],
      columnDefs: [
        {
          className: 'control',
          orderable: false,
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return full?.message ? full.message : full?.story ? full.story : '-';
          },
        },
        {
          targets: 2,
          render: function (data, type, full, meta) {
            return `<span class="badge bg-label-primary me-1">${toLocalDateString(full.created_time)}</span>`;
          },
        },
        {
          targets: -1,
  ...
  $(document).on('click', '.btn-delete', function () {
    ...
    Swal.fire({
      ...
      if (e.value) {
        ...
        $.ajax({
          url: 'facebook/' + $this.data('id') + '?token=' + pageToken,
          type: 'DELETE',
          success: function (res) {
            $.unblockUI();
            Swal.fire({
              ...
              icon: 'success',
              customClass: { confirmButton: 'btn btn-primary waves-effect waves-light' },
            }).then(() => dataPosts.row($this.parents('tr')).remove().draw());
          },
          error: function (e) {
            $.unblockUI();
            const msg = e.responseJSON?.msg || 'There is an error!';
