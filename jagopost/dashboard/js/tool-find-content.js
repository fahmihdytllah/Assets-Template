$(function () {
  ...
  function getContent(url) {
    $('#resultSearch').block({ message: elementLoader, css: { backgroundColor: 'transparent', color: '#fff', border: '0' }, overlayCSS: { backgroundColor: '#fff', opacity: 0.8 } })
    $.ajax({
      data: {
        type: 'get',
  ...
  function showContent(data, type) {
    ...
    if (type === 'article') {
      ...
      $('#resultContent').val(`<p style="text-align: center;"><img src="${data.src.medium}" alt="${data.title} - Jago Post"/></p>`)
