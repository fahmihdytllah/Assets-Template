$(function () {
  ...
  formTools.submit(function (e) {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        loadData(res)
        $('#result').show()
        Swal.fire({ title: 'Good job!', text: 'Successfully checked text plagiarism', icon: 'success', customClass: { confirmButton: 'btn btn-primary' }, buttonsStyling: !1 })
      },
      error: function (e) {
        formTools.unblock()
  ...
  function loadData(data) {
    ...
    $('#pcUnique').html(data.unique)
    $('#pcPlagiarism').html(data.plagiarism)

    data.list_plagiarism.forEach((item, index) => {
      $('#listPlagiarism').append(`<div class="card accordion-item active">
        <h2 class="accordion-header">
          <button type="button" class="accordion-button" data-bs-toggle="collapse" data-bs-target="#accord-${index}" aria-expanded="true">
            <span class="badge rounded-pill bg-label-danger me-1">${item.similarity}</span> ${item.title}
          </button>
        </h2>

        <div id="accord-${index}" class="accordion-collapse collapse show">
          <div class="accordion-body">${item.snippet} <br><a href="${item.link}">Read more</a> </div>
        </div>
      </div>`)
    })
  }
